...
Jaeger automatically records the execution of individual requests across the whole stack of microservices, and presents them as _traces_ consisting of one or more _spans_ (individual calls to a microservice). 

Open the https://jaeger-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Jaeger Query console^] and you should see the _OpenShift Login_ screen. Enter the username and password as below and click _Log In_.
...
To view the traces being generated, click on the **Distributed Tracing** in _Kiali console_  on the left with choosing the following options:

...
Open the http://prometheus-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Prometheus console, window=_blank] and click on *Log in with OpenShift*.
...
Open the http://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Grafana console, window=_blank]
...
image::versions.png[Versions, 700]

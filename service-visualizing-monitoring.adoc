...
Back on http://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Grafana console^], Select the *Istio Service Dashboard*, and then select the `ratings.{{USER_ID}}-bookinfo.svc.cluster.local` service in the _Service_ dropdown:

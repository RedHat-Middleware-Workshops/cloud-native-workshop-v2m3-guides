...
Back on http://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[Grafana console^] and scroll down to the *ratings* service in _Istio Service Dashboard_ and notice that the requests coming from the reviews service have stopped:

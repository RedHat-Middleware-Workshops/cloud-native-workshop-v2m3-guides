...
[IMPORTANT]
====
If you have already deployed the _inventory_ and _catalog_ microservices from Module 1, you can skip this step and move to sectionn "1. Enabling automatic sidecar injection"
====

Incase you haven’t done Module 1 or Module 2 today, or you didn’t quite complete them, you should deploy the coolstore application and microservices by executing the following shell scripts in CodeReady Workspaces Terminal:
...
The following script will deploy the catalog service
[source, shell, role="copypaste"]
----
...
Open {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-inventory/deploymentconfigs/inventory-database/yaml[Inventory Database DeploymentConfig^] and add the following annotation in `spec.template.metadata.annotations` path in the YAML and click on *Save*.
...
image::inventory_db_inject_sidecar.png[istio, 700]

Open {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-inventory/deploymentconfigs/inventory-quarkus/yaml[Inventory Service DeploymentConfig^] and add the following annotation in `spec.template.metadata.annotations` path in the YAML and click on *Save*.
...
image::inventory_inject_sidecar.png[istio, 700]

Next, let's do the same for the catalog and catalog’s database. Open {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-catalog/deploymentconfigs/catalog-database/yaml[Catalog Database DeploymentConfig^] and add the following annotation in `spec.template.metadata.annotations` path in the YAML and click on *Save*.
...
image::catalog_db_inject_sidecar.png[istio, 700]

Now its time to inject the Sidecar container to (Catalog). Let's go ahead and do that. Open {{ CONSOLE_URL }}/k8s/ns/{{ USER_ID }}-catalog/deploymentconfigs/catalog-springboot/yaml[Catalog Service DeploymentConfig^] and add the following annotation in `spec.template.metadata.annotations` path in the YAML and click on *Save*.
...
Let's create a gateway and virtual service to manage ingress traffic by the service mesh. Open the empty `catalog-default.yaml` file in _catalog/rules_ directory to copy the following _Gateway_ and _VirtualService_ in CodeReady Workspaces:
...
Ensure if we injected _side car_ to each pods. Access the https://kiali-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/console/graph/namespaces/?edges=noEdgeLabels&graphType=versionedApp&namespaces={{ USER_ID }}-catalog%2C{{ USER_ID }}-inventory&unusedNodes=false&injectServiceNodes=true&duration=60&pi=15000&layout=dagre[Kiali Graph page^] and check *Traffic Animation* in _Display_ for understanding the traffic flow from _Catalog service_ to _Inventory service_:
...
Open the empty `inventory-default.yaml` file in the _inventory/rules_ directory and copy the following _Gateway_ and _VirtualService_ in CodeReady Workspaces:
...
Now, you can test if the inventory service works correctly via accessing the http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}[CoolStore Inventory page^]:
...
If the Inventory’s front page was set to correctly handle delays, we expect it to load within approximately 5 seconds. To see the web page response times, open the Developer Tools menu in IE, Chrome or Firefox (typically, key combination kbd:[CTRL+SHIFT+I] or kbd:[ALT+Command+I]), select the `Network` tab, and reload the inventory web page.
...
----
    for i in {1..1000} ; do
        curl 'http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}' >& /dev/null &
    done
----

Due to the very conservative circuit breaker, many of these calls will fail with HTTP 503 (Server Unavailable). To see this, open the https://grafana-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/d/LJ_uJAvmk/istio-service-dashboard?orgId=1&refresh=10s&var-service=inventory-quarkus.{{ USER_ID }}-inventory.svc.cluster.local&var-srcns=All&var-srcwl=All&var-dstns=All&var-dstwl=All[Istio Service Dashboard^] in Grafana and ensure to see _Clinet Success Rate(non-5xx responses)_ of inventory service:

[NOTE]
====
It may take 10-20 seconds before the evidence of the circuit breaker is visible within the Grafana dashboard, due to the not-quite-realtime nature of Prometheus metrics and Grafana refresh periods and general network latency.
====
...
[source,sh,role="copypaste"]
----
for i in {1..1000} ; do kill %${i} ; done
----

image::inventory-circuit-breaker-stop.png[circuit-breaker, 700]
...
oc delete virtualservice/inventory-default -n {{ USER_ID }}-inventory && 
oc delete gw/inventory-gateway  -n {{ USER_ID }}-inventory &&
oc create -f $CHE_PROJECTS_ROOT/cloud-native-workshop-v2m3-labs/catalog/rules/catalog-default.yaml -n {{ USER_ID }}-catalog
----
...
We will deploy RH-SSO in a new project. Visit the {{ CONSOLE_URL }}[OpenShift Administrator Console^]. Clcik on **Create Project**:

 * *Name*: `{{ USER_ID }}-rhsso`

image::rhsso-create-project.png[sso, 700]

Switch to the _Dev Console_ and click *+Add* on the left, on the _From Catalog_ box on the *{{ USER_ID }}-rhsso* project overview:

image::sso-catalog.png[sso, 700]

Type in `sso` in the search box, and click on the *Red Hat Single Sign-On 7.2(Ephemeral)*:

image::sso-rhsso.png[sso, 700]
...
* *SSO_SERVICE_PASSWORD*: `{{ OPENSHIFT_USER_PASSWORD }}`

image::rhsso-values.png[sso, 700]

Click on *Create* and you will see that RH-SSO is deployed in the _Topology view_:

image::rhsso-topology.png[sso, 700]

To avoid healthcheck failure randomly, we will remove _liveness_ and _readiness_ healthcheck via running the following command in CodeReady Workspaces Terminal:

[source,sh,role="copypaste"]
----
oc set probe dc/sso --remove --readiness --liveness -n {{ USER_ID }}-rhsso
----

Once this finishes (it may take a minute or two), click on *Open URL* to access RH-SSO web console as below:
...
[NOTE]
====
Add *http://* at the front as well as `/*` at the end of URL.
====

Valid Redirect URIs: http://istio-ingressgateway-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/*
...
- The format is defined in https://tools.ietf.org/html/rfc7519[RFC 7519^].

- You can find more details how https://tools.ietf.org/html/rfc6749[OAuth 2.0^] 

- https://openid.net/connect/[OIDC 1.0^] work in the overall authentication flow.
...
image::rhsso_call_catalog_noauth.png[sso, 700]
...
image::rhsso_web_catalog_noauth.png[sso, 700]
...
Next, open the *application-openshift.properties* in _catalog/src/main/resources_ directory and add the following settings at the bottom of the file:

[source,properties,role="copypaste"]
----
# Set RH-SSO authentication
keycloak.auth-server-url=http://sso-{{ USER_ID }}-rhsso.{{ ROUTE_SUBDOMAIN }}/auth
...
Let’s update *pom.xml* in catalog project to add the needed keycloak dependency to our app:.
...
image::rhsso_catalog_pom_parent.png[sso, 700]
...
image::rhsso_catalog_pom_dependency.png[sso, 700]

Re-build and deploy the project using the following command in a Terminal:

[source,sh,role="copypaste"]
----
mvn clean package spring-boot:repackage -DskipTests -f $CHE_PROJECTS_ROOT/cloud-native-workshop-v2m3-labs/catalog
----
...
* Username or email: *auth{{ USER_ID }}*
* Password: *{{ OPENSHIFT_USER_PASSWORD }}*

image::rhsso_catalog_redirect.png[sso, 700]

Finally, you can access the catalog service as below:

image::rhsso_web_catalog_auth.png[sso, 700]

...
[source,sh,role="copypaste"]
----
oc patch deployments/inventory-database -n {{USER_ID}}-inventory --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
...
oc rollout status -w deployments/inventory-database -n {{USER_ID}}-inventory && \
...
oc patch deployments/catalog-springboot -n {{USER_ID}}-catalog --type='json' -p '[{"op":"add","path":"/spec/template/metadata/annotations", "value": {"sidecar.istio.io/inject": "'"true"'"}}]' && \
oc rollout status -w dc/inventory -n {{USER_ID}}-inventory && \
oc rollout status -w deployments/catalog-springboot -n {{USER_ID}}-catalog
----

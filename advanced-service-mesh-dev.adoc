...
image::catalog_svc_inject_sidecar.png[istio, 700]
...
Ensure if we injected _side car_ to each pods. Access the https://kiali-{{ USER_ID }}-istio-system.{{ ROUTE_SUBDOMAIN }}/console/graph/namespaces/?edges=noEdgeLabels&graphType=versionedApp&namespaces={{ USER_ID }}-catalog%2C{{ USER_ID }}-inventory&unusedNodes=false&injectServiceNodes=true&duration=60&pi=15000&layout=dagre[Kiali Graph page^] and check *{{ USER_ID }}-inventory*, *{{ USER_ID }}-catalog* in _Namespaces_ then check *Traffic Animation* in _Display_ for understanding the traffic flow from _Catalog service_ to _Inventory service_:
...
We will deploy RH-SSO in a new project. Run the following commands in CodeReady Workspaces Terminal:

[source,sh,role="copypaste"]
----
oc new-project {{ USER_ID}}-rhsso &&
oc new-app ccn-sso72 \
   -p SSO_ADMIN_USERNAME=admin \
   -p SSO_ADMIN_PASSWORD=admin \
   -p SSO_REALM=istio \
   -p SSO_SERVICE_USERNAME=auth{{ USER_ID}} \
   -p SSO_SERVICE_PASSWORD={{ OPENSHIFT_USER_PASSWORD }}
----

Visit the {{ CONSOLE_URL }}/topology/ns/{{ USER_ID }}-rhsso[Topology View^]:
...
[source,sh,role="copypaste"]
----
sh $CHE_PROJECTS_ROOT/cloud-native-workshop-v2m3-labs/istio/scripts/cleanup.sh {{ USER_ID }}
----
...
[source,sh,role="copypaste"]
----
oc start-build catalog-springboot --from-file=$CHE_PROJECTS_ROOT/cloud-native-workshop-v2m3-labs/catalog/target/catalog-1.0.0-SNAPSHOT.jar --follow -n {{ USER_ID }}-catalog
----

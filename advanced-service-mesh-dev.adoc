...
In CodeReady, open the empty *ccn-auth-config.yml* file in _catalog/rules_ directory to copy the following _RequestAuthentication_ and _AuthorizationPolicy_:
...
spec:
  ...
  jwtRules:
  - issuer: http://sso-{{ USER_ID }}-rhsso.apps.cluster-ae59.ae59.example.opentlc.com/auth/realms/istio
    jwksUri: http://sso-{{ USER_ID }}-rhsso.apps.cluster-ae59.ae59.example.opentlc.com/auth/realms/istio/protocol/openid-connect/certs
---
apiVersion: security.istio.io/v1beta1

...
[NOTE]
====
These scripts will each take a few minutes to complete. Wait for them to finish!
====

This will build and deploy the _inventory_ and _catalog_ components into their own namespaces. They won’t automatically get Istio sidecar proxy containers yet, but you’ll add that in the next step!
...
First, confirm you have running inventory service and associated database with the following command:
...
----
NAME                         READY   STATUS    RESTARTS   AGE
inventory-database-1-7jt7j   1/1     Running   0          16m
inventory-quarkus-1-wwz89    1/1     Running   0          15m
----
...
[source,sh,role="copypaste"]
----
oc patch dc/inventory-database -n {{USER_ID}}-inventory --type='json' -p '[{"op": "add", "path": "/spec/template/metadata/annotations/sidecar.istio.io~1inject", "value": "'"true"'"}]' && \
oc patch dc/catalog-database -n {{USER_ID}}-catalog --type='json' -p '[{"op": "add", "path": "/spec/template/metadata/annotations/sidecar.istio.io~1inject", "value": "'"true"'"}]' && \
...
[source,sh,role="copypaste"]
----
oc patch dc/inventory-quarkus -n {{USER_ID}}-inventory --type='json' -p '[{"op": "add", "path": "/spec/template/metadata/annotations/sidecar.istio.io~1inject", "value": "'"true"'"}]' && \
oc patch dc/catalog-springboot -n {{USER_ID}}-catalog --type='json' -p '[{"op": "add", "path": "/spec/template/metadata/annotations/sidecar.istio.io~1inject", "value": "'"true"'"}]' && \
oc rollout status -w dc/inventory-quarkus -n {{USER_ID}}-inventory && \
oc rollout status -w dc/catalog-springboot -n {{USER_ID}}-catalog
----

Verify that the `inventory-database` is running with 2 pods (`2/2` in the `READY` column) with this command:
...
----
NAME                         READY   STATUS    RESTARTS   AGE
inventory-database-9-t9gxw   2/2     Running   0          9m25s
inventory-quarkus-2-nhgt6    2/2     Running   0          35m
----
...
Next, let's create a gateway and virtual service to manage ingress traffic by the service mesh. Open the empty `catalog-default.yaml` file in _catalog/rules_ directory to copy the following _Gateway_ and _VirtualService_ in CodeReady Workspaces:
...
[source,sh,role="copypaste"]
----
oc patch -n {{ USER_ID }}-inventory svc/inventory-quarkus -p '{"spec": {"ports":[{"port": 8080, "name": "http"}, {"port": 8443, "name": "https"}]}}' && \
